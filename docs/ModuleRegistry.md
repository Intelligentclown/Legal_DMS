...
| `application.interfaces` | `backend/src/app/application/interfaces/` | Framework-agnostic ports | `AbstractRepository`, `EventBus`, `CommandBus`/`Command`/`CommandHandler`, `QueryBus`/`Query`/`QueryHandler`, `UnitOfWork`/`UnitOfWorkError`, `Cache`, `MetricsService`, `JobQueue`/`Job`, `FileStorage`, `Notifier`, `AuthenticationProvider`/`AuthorizationService`/`CurrentUser`, `AuditLogger`, `SearchIndex`, `FeatureFlagProvider` | Complete for Stage 1 scope (`CommandBus`/`QueryBus`/`UnitOfWork`/`Cache`/`MetricsService` added post-Stage-2, see ADR/0010, ADR/0011, ADR/0012, ADR/0013, ADR/0016) | AI |
...
| `infrastructure.di` | `backend/src/app/infrastructure/di/` | Dependency injection container + startup health check | `Container`, `configure_container()`, `check_container_health()`, `assert_container_healthy()` | Complete (health check added post-Stage-2, wired into `main.py` — ADR/0015) | AI |
...
| `infrastructure.commands` | `backend/src/app/infrastructure/commands/` | Command bus implementation + transaction pipeline | `InMemoryCommandBus`, `TransactionPipelineBehavior` | Complete (default, added post-Stage-2 — ADR/0010, ADR/0012) | AI |
| `infrastructure.queries` | `backend/src/app/infrastructure/queries/` | Query bus implementation | `InMemoryQueryBus` | Complete (default, added post-Stage-2 — ADR/0011) | AI |
| `infrastructure.transactions` | `backend/src/app/infrastructure/transactions/` | Unit of work implementation | `InMemoryUnitOfWork` | Complete (default, non-singleton, no backing resource yet, added post-Stage-2 — ADR/0012) | AI |
| `infrastructure.cache` | `backend/src/app/infrastructure/cache/` | Cache implementation | `InMemoryCache` | Complete (default, singleton, added post-Stage-2 — ADR/0013) | AI |
| `infrastructure.metrics` | `backend/src/app/infrastructure/metrics/` | Metrics service implementation | `LoggingMetricsService` | Complete (default, singleton, no real backend wired, added post-Stage-2 — ADR/0016) | AI |
...
| `infrastructure.modules` | `backend/src/app/infrastructure/modules/` | Plugin/module registry + manifest loader | `AppModule`, `ModuleRegistry`, `registry`, `ModuleManifest`/`ModuleManifestEntry`/`ModuleManifestLoader`/`ModuleManifestError` | Complete (empty registry; manifest loader added post-Stage-2, not wired into `main.py` yet — ADR/0014) | AI |
