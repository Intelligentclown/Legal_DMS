...
| T75 | Current-user display + logout action in `MainLayout`'s header. **Authorized by the project owner, 2026-08-20, recorded here — as its own documentation-only commit — before any implementation exists.** Approved scope: Implement the current authenticated-user display and logout action in MainLayout's header using the existing AuthProvider state and logout functionality. Wire the existing ipcBridge.clearRefreshToken() into the user-facing logout flow to complete the deferred secure-session clearing from T74. T76+ functionality, automatic refresh-token exchange/retry/rotation, or new authentication mechanisms are explicitly out of scope. Not yet implemented. | XS | T70 |
