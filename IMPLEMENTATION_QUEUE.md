...
| ID | Task | Complexity | Depends on |
|---|---|---|---|
| T77 | Gate `/docs`/`/redoc` behind `settings.is_development` (Stage 2.5's F4 — bundled now since API docs exposure is meaningfully more sensitive once real auth/user data exists). **Authorized by the project owner, 2026-08-20, recorded here — as its own documentation-only commit — before any implementation exists.** Approved scope: Gate /docs and /redoc based on the existing settings.is_development configuration. Reuse the existing settings/configuration mechanism. Do not introduce new authentication mechanisms. Do not modify unrelated API routes. Do not implement T78+ or any other future task. | XS | — |
