...
| T56 | Update `presentation/api/deps.py`'s `CurrentUserDep` for the new provider signature. **Authorized by the project owner, 2026-08-10, recorded here — as its own documentation-only commit — before any implementation exists.** Scope strictly limited to this row's own description: extracting the real bearer token from the incoming request in place of `get_current_user()`'s current `token=None` placeholder. `T57` and any route remain explicitly unauthorized. Not yet implemented. | XS | T55 |
...
consecutive occurrence, `T52`/`T53`/`T54`/`T55`), not erased. `T56`/`T57` remain not
